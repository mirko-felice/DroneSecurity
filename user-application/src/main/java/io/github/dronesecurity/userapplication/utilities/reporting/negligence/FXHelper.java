...
public final class FXHelper {
    ...
    private static <T extends NegligenceReportWithID> @NotNull
        ...
        final Consumer<GenericUser> userConsumer = user ->
                Platform.runLater(() -> controller.updateDetails(user));

        final Consumer<String> courierConsumer = value ->
                UserAPIHelper.get(UserAPIHelper.Operation.RETRIEVE_COURIER_BY_USERNAME).onSuccess(res ->
                        userConsumer.accept(Json.decodeValue(res.bodyAsJsonObject().toBuffer(), GenericUser.class)));
        ...
        final Consumer<String> maintainerConsumer = value ->
                UserAPIHelper.get(UserAPIHelper.Operation.RETRIEVE_MAINTAINER_BY_USERNAME).onSuccess(res ->
                        userConsumer.accept(Json.decodeValue(res.bodyAsJsonObject().toBuffer(), GenericUser.class)));
